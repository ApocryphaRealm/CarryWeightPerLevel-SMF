...
SKSEPluginLoad(const SKSE::LoadInterface* a_skse)
{
	...
	// The SerializationInterface is a core SKSE interface (unlike DevBench's, which arrives via
	// another plugin's own messaging listener), so it can be registered here immediately rather
	// than waiting for any SKSE message.
	persistence::Init();
